...
namespace Drupal\tawjeeh_people\Plugin\EntityReferenceSelection;
 ...
 *   id = "tawjeeh_people_advanced_user_selection",
 *   label = @Translation("Advanced"),
 *   group = "tawjeeh_people_advanced_user_selection",
 *   entity_types = {"user"},
 *   weight = 0
 * )
 */
class UserSelection extends DefaultSelection {
  ...
  public function getReferenceableEntities($match = NULL, $match_operator = 'CONTAINS', $limit = 0) {
    ...
    foreach ($entities as $entity_id => $entity) {
      ...
      $name = $entity->get('field_user_full_name')->value;
      /**
       * @var \Drupal\taxonomy\Entity\Term $branch
       */
      $branch = $entity->get('field_user_branch')->entity->getName();
      $options[$bundle][$entity_id] = Html::escape("$name ($branch)");
    }
  ...
  protected function buildEntityQuery($match = NULL, $match_operator = 'CONTAINS') {
    ...
    if ($match) {
      $name = $query->andConditionGroup()->condition('field_user_full_name', $match, $match_operator);
      $branch = $query->andConditionGroup()->condition('field_user_branch.entity:taxonomy_term.name', $match, $match_operator);
